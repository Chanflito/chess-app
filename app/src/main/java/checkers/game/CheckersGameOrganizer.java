...
public class CheckersGameOrganizer implements GameOrganizer {
    ...
    public CheckersGameOrganizer(GameData gameData, GameMover gameMover, TurnHandler turnHandler, GameOverCondition gameOverCondition) {
    ...
    public Result<?,?> move(Movement movement, GameData gameData){
        ...
        if (isWinner.key() && isWinner.value().isPresent()){
    ...
    public GameOrganizer copy() {
        return new CheckersGameOrganizer(this.gameData.copy(), this.gameMover, this.turnHandler, this.gameOverCondition);
    }
    ...
    private MoveResult<GameOrganizer, String> getResultOfNotLegitMove(Result<GameData, String> gameResult) {
        return new MoveResult<>(new CheckersGameOrganizer(gameResult.key(), gameMover, turnHandler, this.gameOverCondition)
                , Optional.of(gameResult.value().get()));
    }

    @NotNull
    private MoveResult<GameOrganizer, String> getResultOfLegitMove(Result<GameData, String> gameResult, Position finalPosition){
        TurnHelper turnHelper=new TurnHelper(finalPosition,gameResult.key().getBoard().copy());
        ...
        return new MoveResult<>(new
                CheckersGameOrganizer(gameResult.key(), gameMover, nextTurn, this.gameOverCondition), Optional.empty());
    }
